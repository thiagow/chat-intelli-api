...
- **[router/agent-router.service.ts](src/modules/ai-agents/router/agent-router.service.ts)** — picks the agent **deterministically**, no LLM involved: an in-flight conversation keeps its `activeAgentId`; otherwise the channel's AUTONOMOUS `ORCHESTRATOR` (ordered by `createdAt`), falling back to the first AUTONOMOUS worker. `shouldHandle()` gates the run on the tri-state `aiEnabled` cascade (conversation → channel → org), business hours and `aiMonthlyTokenCap`. **[classifier/](src/modules/ai-agents/classifier/) is dead code** — `ClassifierModule` is not imported by `AiAgentsModule`, so `AiAgentRun.classifiedIntent` / `skippedOrchestrator` are always null/false. The LLM pre-classification step was removed because its intent→agent map was hardcoded with another operation's names and never matched, so 100% of traffic reached the orchestrator anyway while paying an extra LLM call. Specialization is now decided by the orchestrator itself via `listAvailableAgents` + `delegateToAgent`.
...
- **[llm/](src/modules/ai-agents/llm/)** — two providers, Sakana and OpenAI, both via the OpenAI-compatible SDK. `AUX_LLM_PROVIDER` selects the provider for agent-less services (memory extraction, eval judge, RAG reranker — and the classifier, while it stays wired out) since those have no `modelId` to infer from.
- **[prompts/](src/modules/ai-agents/prompts/)** — four composable layers in fixed order: `security` (immutable, tenant isolation + anti-injection), `personality`, `capabilities`, `context`. **Only `security` reaches production.** `PromptComposerService` is injected solely by the eval runner; the production path is [runner/prompt-builder.service.ts](src/modules/ai-agents/runner/prompt-builder.service.ts) (a ~380-line Eta template) with the security layer prepended and RAG appended in `augmentSystemPromptWithLayers`. So editing `personality`/`capabilities`/`context.layer.ts` changes what evals score, not what customers see — the two paths assemble different prompts.
...
- **[confirmations/](src/modules/ai-agents/confirmations/)** — skills flagged as needing approval (`AiAgentSkill.requiresApproval`, set per `(agent, skill)` pair) create an `AiPendingAction` and execute later via queue instead of running inline. `transferToHuman` always takes this path: it does **not** transfer on the spot — it queues a `critical` pending action and returns `status: queued_for_processing` plus an `agent_should_say` script, while still reporting `finalAction: TRANSFERRED_TO_HUMAN` so the loop stops.
...
[docs/orchestration-improvements-from-bullq.md](docs/orchestration-improvements-from-bullq.md) records the design rationale behind the classifier / confidence routing / layered prompts work — read it as history, not as a description of the running system (see the classifier and prompts notes above).

For prompt authoring — the six sources that make up a run's system prompt, which field to write in, the ~250 hardcoded lines the platform already injects, and the runtime guards that discard model output — load the `chat-intelli-ai-agents` skill at the workspace root, or read [docs/ai-agents.md](docs/ai-agents.md).
